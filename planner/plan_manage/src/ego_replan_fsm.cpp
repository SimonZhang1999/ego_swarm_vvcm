...
namespace ego_planner
{
  ...
  void EGOReplanFSM::init(rclcpp::Node::SharedPtr &node)
  {
    ...
    node_->declare_parameter("fsm/default_goal_z", 0.6);
    node_->declare_parameter("fsm/planar_mode", false);
    ...
    node_->get_parameter("fsm/default_goal_z", default_goal_z_);
    node_->get_parameter("fsm/planar_mode", planar_mode_);
  ...
  void EGOReplanFSM::planNextWaypoint(const Eigen::Vector3d next_wp)
  {
    if (!have_odom_)
    {
      pending_target_ = next_wp;
      have_pending_target_ = true;
      RCLCPP_WARN(node_->get_logger(), "Goal received before odom is ready; defer planning.");
      return;
    }
    ...
    if (planar_mode_)
    {
      start_pos(2) = default_goal_z_;
      start_vel(2) = 0.0;
      target(2) = default_goal_z_;
    }
    success = planner_manager_->planGlobalTraj(start_pos, start_vel, Eigen::Vector3d::Zero(), target, Eigen::Vector3d::Zero(), Eigen::Vector3d::Zero());
  ...
  void EGOReplanFSM::waypointCallback(const std::shared_ptr<const geometry_msgs::msg::PoseStamped> &msg)
  {
    ...
    const double goal_z = planar_mode_ ? default_goal_z_ : (msg->pose.position.z > 0.05 ? msg->pose.position.z : default_goal_z_);
    Eigen::Vector3d end_wp(msg->pose.position.x, msg->pose.position.y, goal_z);

    if (!have_odom_)
    {
      pending_target_ = end_wp;
      have_pending_target_ = true;
      RCLCPP_WARN(node_->get_logger(), "Goal received before odom is ready; defer planning.");
      return;
    }
  ...
  void EGOReplanFSM::odometryCallback(const std::shared_ptr<const nav_msgs::msg::Odometry> &msg)
  {
    ...
    odom_pos_(2) = planar_mode_ ? default_goal_z_ : msg->pose.pose.position.z;

    odom_vel_(0) = msg->twist.twist.linear.x;
    odom_vel_(1) = msg->twist.twist.linear.y;
    odom_vel_(2) = planar_mode_ ? 0.0 : msg->twist.twist.linear.z;
  ...
  void EGOReplanFSM::execFSMCallback()
  {
    ...
    case WAIT_TARGET:
    {
      if (have_pending_target_ && have_odom_)
      {
        Eigen::Vector3d pending_target = pending_target_;
        have_pending_target_ = false;
  ...
  void EGOReplanFSM::enforcePlanarLocalTraj()
  {
    if (!planar_mode_)
      return;
    ...
    auto info = &planner_manager_->local_data_;
    Eigen::MatrixXd pos_pts = info->position_traj_.getControlPoint();
    if (pos_pts.rows() < 3 || pos_pts.cols() == 0)
      return;

    pos_pts.row(2).setConstant(default_goal_z_);
    UniformBspline planar_traj(pos_pts, 3, info->position_traj_.getInterval());
    planar_traj.setKnot(info->position_traj_.getKnot());

    info->position_traj_ = planar_traj;
    info->velocity_traj_ = info->position_traj_.getDerivative();
    info->acceleration_traj_ = info->velocity_traj_.getDerivative();
    info->start_pos_(2) = default_goal_z_;
