...
namespace ego_planner
{
  ...
  public:
    ...
    void enforcePlanarControlPoints();
  ...
  private:
    ...
    bool formation_topology_enabled_{false};
    double formation_topology_weight_{0.35};
    double formation_topology_edge_weight_{1.0};
    double formation_topology_diag_weight_{0.25};
    double formation_topology_length_{2.4};
    double formation_topology_width_{2.0};
...
#define INIT_min_ellip_dist_ 123456789.0123456789
    ...
    void calcFormationTopologyCost(const Eigen::MatrixXd &q, double &cost, Eigen::MatrixXd &gradient);
    bool collectFormationTopologyPoints(const Eigen::MatrixXd &q, int ctrl_pt_idx, double glb_time,
                                        std::array<Eigen::Vector3d, 4> &points);
    void addTopologyDirectedDistanceCost(const std::array<Eigen::Vector3d, 4> &points, int self_id, int self_ctrl_idx,
                                         int id_a, int id_b, const Eigen::Vector3d &target_offset, double weight,
                                         double &cost, Eigen::MatrixXd &gradient) const;
