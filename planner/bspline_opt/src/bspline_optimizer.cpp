...
namespace ego_planner
{
  ...
  void BsplineOptimizer::setParam(rclcpp::Node::SharedPtr node)
  {
    ...
    node->declare_parameter("optimization/planar_mode", false);
    node->declare_parameter("optimization/fixed_z", 0.35);
    node->declare_parameter("optimization/formation_topology/enabled", false);
    node->declare_parameter("optimization/formation_topology/weight", 0.35);
    node->declare_parameter("optimization/formation_topology/edge_weight", 1.0);
    node->declare_parameter("optimization/formation_topology/diag_weight", 0.25);
    node->declare_parameter("optimization/formation_topology/length", 2.4);
    node->declare_parameter("optimization/formation_topology/width", 2.0);
    ...
    node->get_parameter("optimization/planar_mode", planar_mode_);
    node->get_parameter("optimization/fixed_z", fixed_z_);
    node->get_parameter("optimization/formation_topology/enabled", formation_topology_enabled_);
    node->get_parameter("optimization/formation_topology/weight", formation_topology_weight_);
    node->get_parameter("optimization/formation_topology/edge_weight", formation_topology_edge_weight_);
    node->get_parameter("optimization/formation_topology/diag_weight", formation_topology_diag_weight_);
    node->get_parameter("optimization/formation_topology/length", formation_topology_length_);
    node->get_parameter("optimization/formation_topology/width", formation_topology_width_);
  }
  ...
  {
    cps_.points = points;
    enforcePlanarControlPoints();
  }

  void BsplineOptimizer::enforcePlanarControlPoints()
  {
    if (planar_mode_ && cps_.points.rows() >= 3)
    {
      cps_.points.row(2).setConstant(fixed_z_);
    }
  }
  ...
  std::vector<std::pair<int, int>> BsplineOptimizer::initControlPoints(Eigen::MatrixXd &init_points, bool flag_first_init /*= true*/)
  {
    ...
    if (planar_mode_)
    {
      init_points.row(2).setConstant(fixed_z_);
      enforcePlanarControlPoints();
    }
  ...
  bool BsplineOptimizer::collectFormationTopologyPoints(const Eigen::MatrixXd &q, int ctrl_pt_idx, double glb_time,
                                                        std::array<Eigen::Vector3d, 4> &points)
  {
    if (drone_id_ < 0 || drone_id_ >= 4 || !swarm_trajs_ || swarm_trajs_->size() < 4)
      return false;

    points[drone_id_] = q.col(ctrl_pt_idx);
    if (planar_mode_)
      points[drone_id_](2) = fixed_z_;
    ...
    for (int id = 0; id < 4; ++id)
    {
      ...
      auto &peer = swarm_trajs_->at(id);
      if (peer.drone_id != id)
        return false;

      const double peer_start_time = peer.start_time_.seconds();
      const double local_time = glb_time - peer_start_time;
      if (local_time < 0.0 || local_time > peer.duration_ - 0.05)
        return false;

      points[id] = peer.position_traj_.evaluateDeBoorT(local_time);
      if (planar_mode_)
        points[id](2) = fixed_z_;
    }

    return true;
  }

  void BsplineOptimizer::addTopologyDirectedDistanceCost(const std::array<Eigen::Vector3d, 4> &points,
                                                        int self_id, int self_ctrl_idx,
                                                        int id_a, int id_b, const Eigen::Vector3d &target_offset,
                                                        double weight, double &cost,
                                                        Eigen::MatrixXd &gradient) const
  {
    if (self_id != id_a && self_id != id_b)
      return;

    Eigen::Vector3d err = (points[id_b] - points[id_a]) - target_offset;
    if (planar_mode_)
      err(2) = 0.0;

    cost += weight * err.squaredNorm();
    const Eigen::Vector3d grad_b = 2.0 * weight * err;

    if (self_id == id_a)
      gradient.col(self_ctrl_idx) -= grad_b;
    else
      gradient.col(self_ctrl_idx) += grad_b;
  }

  void BsplineOptimizer::calcFormationTopologyCost(const Eigen::MatrixXd &q, double &cost, Eigen::MatrixXd &gradient)
  {
    cost = 0.0;
    if (!formation_topology_enabled_ || formation_topology_weight_ <= 0.0 || drone_id_ < 0 || drone_id_ >= 4)
      return;

    const double length = std::max(0.1, formation_topology_length_);
    const double width = std::max(0.1, formation_topology_width_);
    const Eigen::Vector3d edge_x(length, 0.0, 0.0);
    const Eigen::Vector3d edge_y(0.0, width, 0.0);
    const Eigen::Vector3d diag_forward(length, width, 0.0);
    const Eigen::Vector3d diag_backward(-length, width, 0.0);
    const int end_idx = q.cols() - order_;
    const double t_now = rclcpp::Clock().now().seconds();

    for (int i = order_; i < end_idx; ++i)
    {
      const double glb_time = t_now + ((double)(order_ - 1) / 2 + (i - order_ + 1)) * bspline_interval_;
      std::array<Eigen::Vector3d, 4> points;
      if (!collectFormationTopologyPoints(q, i, glb_time, points))
        continue;

      addTopologyDirectedDistanceCost(points, drone_id_, i, 0, 1, edge_x, formation_topology_edge_weight_, cost, gradient);
      addTopologyDirectedDistanceCost(points, drone_id_, i, 2, 3, edge_x, formation_topology_edge_weight_, cost, gradient);
      addTopologyDirectedDistanceCost(points, drone_id_, i, 0, 2, edge_y, formation_topology_edge_weight_, cost, gradient);
      addTopologyDirectedDistanceCost(points, drone_id_, i, 1, 3, edge_y, formation_topology_edge_weight_, cost, gradient);
      addTopologyDirectedDistanceCost(points, drone_id_, i, 0, 3, diag_forward, formation_topology_diag_weight_, cost, gradient);
      addTopologyDirectedDistanceCost(points, drone_id_, i, 1, 2, diag_backward, formation_topology_diag_weight_, cost, gradient);
    }
  }
...
// #define USE_SECOND_CLEARENCE_CHECK
  ...
  void BsplineOptimizer::combineCostRebound(const double *x, double *grad, double &f_combine, const int n)
  {
    ...
    double f_smoothness, f_distance, f_feasibility /*, f_mov_objs*/, f_swarm, f_topology, f_terminal;
    ...
    Eigen::MatrixXd g_topology = Eigen::MatrixXd::Zero(3, cps_.size);
    ...
    calcFormationTopologyCost(cps_.points, f_topology, g_topology);
    ...
    f_combine = lambda1_ * f_smoothness + new_lambda2_ * f_distance + lambda3_ * f_feasibility +
                new_lambda2_ * f_swarm + formation_topology_weight_ * f_topology + lambda2_ * f_terminal;
    ...
    Eigen::MatrixXd grad_3D = lambda1_ * g_smoothness + new_lambda2_ * g_distance + lambda3_ * g_feasibility +
                              new_lambda2_ * g_swarm + formation_topology_weight_ * g_topology + lambda2_ * g_terminal;
    if (planar_mode_)
    {
